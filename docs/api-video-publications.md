...
| `tags` | string[] | 标签列表 |
| `ext_info` | object | 扩展信息，承载带货口令和外部商品列表 |
| `ext_info.promotion_code` | string | 8 位数字带货口令 |
| `ext_info.ext_products` | object[] | 外部商品列表，无商品时为空数组 |
...
#### ext_info.ext_products 对象字段

来源于图搜接口 `/style-outfits/search/image-internal` 返回的 `candidates`，附加 AI 单品信息后透传。

| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `id` | number | 候选序号 |
| `position` | int/null | 外部搜图排名，内部结果为 null |
| `title` | string | 商品标题，内部结果为空字符串 |
| `link` | string | 商品页面链接 |
| `source` | string | 来源平台名称，内部结果为空字符串 |
| `thumbnail` | string | 商品缩略图 URL |
| `image` | string | 商品主图 URL |
| `price.value` | string | 价格原始文本 |
| `price.extracted_value` | number | 提取的价格数值 |
| `price.currency` | string | 货币符号 |
| `tier` | int/null | 正品网站可信度，1 最高，内部结果为 null |
| `tier_label` | string | 可信度展示文案，内部结果为空字符串 |
| `product_code` | string | 内部商品 SPU code，外部结果为空字符串 |
| `sku_code` | string | 内部商品 SKU code，外部结果为空字符串 |
