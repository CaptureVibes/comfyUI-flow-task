...
async def _upload_video_file(file_path: str, filename: str) -> str:
    ...
    实际后端由 VIDEO_UPLOAD_BACKEND 决定：
      - gcs（默认）：上传到 GCS bucket，返回 public URL
      - cdn：POST 到旧 upload API，返回 CDN URL
    """
...
async def _download_video(source_url: str, out_path: str) -> str:
    ...
    if _is_tiktok_url(source_url):
        from app.utils.tiktok_download import download_to_file
        return await download_to_file(source_url, out_path)
...
async def _do_download_and_upload_inner(vs_id: UUID) -> None:
    ...
    while attempt < _MAX_ATTEMPTS:
        ...
        try:
            ...
            from app.utils.tiktok_download import SlideshowNotDownloadable
            is_apify_404 = (
                isinstance(exc, _httpx.HTTPStatusError)
                and exc.response.status_code == 404
                and "api.apify.com" in str(exc.request.url)
            )
            is_slideshow = isinstance(exc, SlideshowNotDownloadable)
            if is_apify_404 or is_slideshow or attempt >= _MAX_ATTEMPTS:
                if is_slideshow:
                    logger.error("Video %s slideshow 帖（无视频文件），不再重试", vs_id, exc_info=exc)
                elif is_apify_404:
